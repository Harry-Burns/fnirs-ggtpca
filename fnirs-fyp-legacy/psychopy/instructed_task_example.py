...
# --- Test Parameters
BASELINE_DUR = 5.0 #20.0 # Initial & final baseline duration

TASK_DUR = 8.0 #10.0 # Task duration
REST_DUR = 5.0 #20.0 # Rest duration

N_BLOCKS = 2 # 8 # Number of blocks per trial

#TRIALS = ["NHM", "SHM", "LHM"]
TRIALS = ["NHM", "SHM", "LHM"]

# Motion Params (seconds)
SHM_MOTION_DUR = 1.5 #1.0 # SHM motion stim duration
LHM_MOTION_DUR = 1.5 #1.0 # LHM motion stim duration
MOTION_PADDING = (3.0, 2.0) # Padding (before,after)
...
def main(args):
    ...
    win = visual.Window(WINDOW_RES, color="black", units="pix", fullscr=False) #monitor='testMonitor',screen=1)

    # -- defining window text
    instr_text = visual.TextStim(win, text="Finger Tapping Task (EXAMPLE)\n\nEnsure you have read the instructions in the powerpoint.\n\nOnce you are ready, press ENTER to start.", color="white")
    ...
    def show_stim(stims,sample=None,wait_dur=None):
        ...
        win.flip()
        if args.uselsl and sample is not None: outlet.push_sample([sample])
